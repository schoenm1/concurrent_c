...
LOG_TRACE(LOG_INFORMATIONAL,"Client can connect to Server on Port %i", ServerPort);
	...
	while (TRUE) {
		...
		handle_error(clientSocket, "accept() failed", PROCESS_EXIT);

		/* clntSock is connected to a client! */
		printf("Warning = %i\t chosen Loglevel = %i\n", LOG_WARNING, LOGLEVEL);
		LOG_TRACE(LOG_WARNING, "Test Log Warning\n");

		LOG_TRACE(LOG_NOTICE, "Handling Client %s\n", inet_ntoa(ClientSocketAddress.sin_addr));

		/* Handle Client and PThread of Client */

		pthread_t thread;
		int *client_socket_ptr = (int *) malloc(sizeof(int));
		*client_socket_ptr = clientSocket;
		pthread_create(&thread, NULL, handle_tcp_client, client_socket_ptr);
		LOG_TRACE(LOG_INFORMATIONAL, "Created new PThread for a Client...");
...
void* handle_tcp_client(void *client_socket_ptr) {
	...
	while (TRUE) {
		...
		LOG_TRACE(LOG_INFORMATIONAL, "Server is now ready for reveicing messages from Clients.");
		recvMsgSize = recv(client_socket, recBuffer, BUFSIZE - 1, 0);
		...
		LOG_TRACE(LOG_INFORMATIONAL, "Received Message Size = %i", recvMsgSize);
		LOG_TRACE(LOG_INFORMATIONAL, "rec Buffer = %s", recBuffer);
